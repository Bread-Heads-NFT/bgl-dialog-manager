...
import { BGL_DIALOG_MANAGER_PROGRAM_ID } from '../src';
...
test('it registers the program', async (t) => {
  ...
  const program = umi.programs.get('bglDialogManager');

  // Then we expect it to be the same as the program ID constant.
  t.true(samePublicKey(program.publicKey, BGL_DIALOG_MANAGER_PROGRAM_ID));
});
